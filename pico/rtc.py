...
class DS3231(RTCC):
 ...
 def temperature(self):
   _data = bytearray(1)
   _data = self.i2c.readfrom_mem(self.addr, 0x11, 2, addrsize=8)
   _upper = _data[0]
 ...
 def offset(self,_value=None):
  if _value==None:
   _data = bytearray(1)
   _data = self.i2c.readfrom_mem(self.addr, 0x10, 1, addrsize=8)      
   return (_data[0])
  else:
   _reg = bytearray(1)
   _reg[0]=_value
   self.i2c.writeto_mem(self.addr, 0x10, _reg, addrsize=8)
   utime.sleep_ms(50)
